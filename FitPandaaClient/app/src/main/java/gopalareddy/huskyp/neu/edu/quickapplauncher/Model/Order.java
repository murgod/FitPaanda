package gopalareddy.huskyp.neu.edu.quickapplauncher.Model;
...
public class Order implements Parcelable {
   ...
   // public Customer customer = new Customer();
    ...
    public void generateTotalBill(){
        double totalBill = this.burgerCount*this.burgerPrice +
                           this.chickensCount*this.chickensPrice +
                                this.friesCount*this.friesPrice +
                                   this.ringsCount*this.ringsPrice;
        this.totalBill = totalBill + (0.3*totalBill); // Total + 30% profit,
    }
    public double getTaxmount(){
        return (0.06 * getTotalBill());
    }

    public double TotalBill(){
        return (getTaxmount() + getTotalBill());
    }
    ...
    public String toString() {
        return "ORDER DETAILS" + "\n" +
                "Burgers=   "      +getBurgerCount() +"\n" +
                "Chicken=   "      +getChickensCount() +"\n"+
                "French Fries=   " +getFriesCount() +"\n"+
                "Onion Rings=   "  +getRingsCount() +"\n"+
                "Sub total=    "   +getTotalBill()+"\n"+
                "6% sales Tax=    "+getTaxmount()+"\n"+
                "Total=   "        +TotalBill()+ "\n";
    }
